...
const MasterDataPopUp: React.FC<MasterDataPopUpProps> = ({
  ...
  const form = useForm({
    ...
    validate: {
      ...
      masterDataName: (value) => {
        ...
        if (value.length > 200) return 'Tên loại dữ liệu không được quá 200 ký tự';
        return null;
      },
      sortNo: (value) => {
  ...
  const title = (!data?.masterDataCode ? 'Thên Mới' : 'Cập Nhật') + ' Loại Dữ Liệu';
  ...
  const handleOnSubmit = async (values: any) => {
    ...
    try {
      let response;
      if (isEditMode) {
        response = await axiosInstance.patch(`/master-data/updateMasterDataByCode/${data?.masterDataCode}`, {
          ...values,
  ...
  return (
    <Modal opened={opened} onClose={onClose} title={title} size="md" centered style={{ position: 'relative' }}>
      ...
      <form onSubmit={form.onSubmit((values) => handleOnSubmit(values))}>
        ...
        >
          ...
          />
          <Button type="submit" variant="outline">Lưu dữ liệu</Button>
        </Stack>
      </form>
    </Modal>
  );
